...
public:
	...
	BECK_API void CreateGraph();

	/** Clears all nodes from the graph (for old editor's buffer soundcue) */
	BECK_API void ClearGraph();

	/** Set up EdGraph parts of a SoundNode */
	BECK_API void SetupMovesetNode(class UMovesetNode_Base* InMoveNode, bool bSelectNewNode = true);

	/** Use the SoundCue's children to link EdGraph Nodes together */
	BECK_API void LinkGraphNodesFromMoveNodes();

	/** Use the EdGraph representation to compile the SoundCue */
	BECK_API void CompileMoveNodesFromGraphNodes();

	/** Get the EdGraph of SoundNodes */
	BECK_API class UEdGraph* GetGraph();

	/** Sets the moveset graph editor implementation.* */
	static BECK_API void SetMovesetNodeEditor(TSharedPtr<IMovesetNodeEditor> InMovesetGraphEditor);

	/** Gets the moveset graph editor implementation. */
	static TSharedPtr<IMovesetNodeEditor> BECK_API GetMovesetNodeEditor();
#endif
...
private:
	...
	static BECK_API TSharedPtr<IMovesetNodeEditor> MovesetMoveEditor;
#endif
};
