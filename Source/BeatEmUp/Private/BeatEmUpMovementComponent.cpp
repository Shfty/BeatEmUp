...
#include "BeatEmUp.h"
#include "BeatEmUpMovementComponent.h"

UBeatEmUpMovementComponent::UBeatEmUpMovementComponent(const FObjectInitializer& ObjectInitializer)
...
void UBeatEmUpMovementComponent::InitializeComponent()
{
...
void UBeatEmUpMovementComponent::TickMovement(float DeltaTime)
{
...
FVector UBeatEmUpMovementComponent::ComputeMoveDelta(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UBeatEmUpMovementComponent::ComputeAcceleration(const FVector& InVelocity, float DeltaTime) const
{
...
FVector UBeatEmUpMovementComponent::ComputeVelocity(FVector InitialVelocity, float DeltaTime) const
{
	// v = v0 + a*t
...
FVector UBeatEmUpMovementComponent::LimitVelocity(FVector NewVelocity) const
{
...
bool UBeatEmUpMovementComponent::HandleDeflection(FHitResult& Hit, const FVector& OldVelocity, float& SubTickTimeRemaining)
{
...
bool UBeatEmUpMovementComponent::HandleSliding(FHitResult& Hit, float& SubTickTimeRemaining)
{
...
void UBeatEmUpMovementComponent::UpdateGrounded()
{
	...
	FCollisionQueryParams TraceParams = FCollisionQueryParams(FName(TEXT("BeatEmUpMovement Ground Check")), true, this->GetOwner());
...
bool UBeatEmUpMovementComponent::IsMovingOnGround() const
{
	return bIsGrounded;
}

float UBeatEmUpMovementComponent::GetGravityZ() const
{
	return ShouldApplyGravity() ? Super::GetGravityZ() * GravityScale : 0.f;
}

void UBeatEmUpMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed)
{
...
void UBeatEmUpMovementComponent::RequestPathMove(const FVector& MoveInput)
{
...
void UBeatEmUpMovementComponent::AddImpulse(FVector Impulse, bool bVelocityChange)
{
	if(!Impulse.IsZero())
	{
		...
		if(!bVelocityChange)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Attempt to apply impulse to zero or negative Mass in BeatEmUpMovement"));
			}
		}
...
void UBeatEmUpMovementComponent::AddForce(FVector Force, bool bAccelChange)
{
	if(!Force.IsZero())
	{
		// handle scaling by mass
		...
		if(!bAccelChange)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("Attempt to apply force to zero or negative Mass in BeatEmUpMovement"));
			}
		}
...
void UBeatEmUpMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
